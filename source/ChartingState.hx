...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	//var useTrident:Bool = false;
	var useTrident:Int = 0; // 0 for normal notes. 1 for Hexley (RED, HEALTH DRAIN) tridents. 2 for Beastie/Trolling (GOLD, MUST HIT OR DIE) tridents.
	static var useTridentMax = 2;
	...
	function addSongUI():Void
	{
		...
		{
			// pre lowercasing the song name (reloadSongJson)
			var songLowercase = StringTools.replace(_song.song, " ", "-").toLowerCase();
				switch (songLowercase) {
					case 'dad-battle': songLowercase = 'dadbattle';
					case 'philly-nice': songLowercase = 'philly';
				}
			loadJson(songLowercase);
		});


		var restart = new FlxButton(10,140,"Reset Chart", function()
            {
							if (curSong != 'True Trolling' && curSong != 'True Trolling (Duet)')
							{
		...
		noteStyleDropDown.dropDirection = Down;
	...
	override function update(elapsed:Float)
	{
		...
		//snapText.text = "Snap: 1/" + snap + " (" + (doSnapShit ? "Control to disable" : "Snap Disabled, Control to renable") + ")\nAdd Notes: 1-8 (or click)\nTrident Note Placement ( " + (useTrident ? "T to disable" : "Trident Placing Disabled, T to renable") + ")\n";

		var tridentText:String = "(Trident Placing Disabled, T to renable)";

		switch (useTrident)
		{
			case 0:
				tridentText = "(Trident Placing Disabled, T to renable)";
			case 1:
				tridentText = "(Using RED helth-drain tridents, T to use GOLD)";
			case 2:
				tridentText = "(Using GOLD death-on miss-tridents, T to disable)";
		}
		...
		if (!typingShit.hasFocus)
		{
			...
			if (FlxG.keys.justPressed.R && curSong != 'True Trolling' && curSong != 'True Trolling (Duet)')
			{
				if (FlxG.keys.pressed.SHIFT)
					resetSection(true);
				else
	...
	function updateGrid():Void
	{
		...
		for (i in sectionInfo)
		{
			...
			if (16 > daNoteInfo && daNoteInfo >= 8)
			{
				daFakeNoteInfo = daNoteInfo - 8;
			}
			else if (24 > daNoteInfo && daNoteInfo >= 16) // this needs to be hard-coded like this or it won't work
			{
				daFakeNoteInfo = daNoteInfo - 16;
			}

			note.x = Math.floor(daFakeNoteInfo * GRID_SIZE);
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if (i.strumTime == note.strumTime && i.trueNoteData == note.trueNoteData)
			{
	...
	function deleteNote(note:Note):Void
		{
			...
			for (i in _song.notes[curSection].sectionNotes)
			{
				if (i[0] == note.strumTime && i[1] == note.trueNoteData)
				{
	...
	private function addNote(?n:Note):Void
	{
		...
		if (useTrident > 0 && noteData < 8) // I forget why I did the second half of this. Not gonna touch it.
		{
			trace(noteData);

			noteData = Math.floor(FlxG.mouse.x / GRID_SIZE) + (8 * useTrident);

			//switch (useTrident)
			//{

				//case 1:
					//noteData = Math.floor(FlxG.mouse.x / GRID_SIZE) + 8;

				//case 2:
					//noteData = Math.floor(FlxG.mouse.x / GRID_SIZE) + 16;
		...
		if (n != null)
			_song.notes[curSection].sectionNotes.push([n.strumTime, n.trueNoteData, n.sustainLength]);
		else
	...
	private function saveLevel()
	{
		...
		if ((data != null) && (data.length > 0))
		{
			...
			_file.save(data.trim(), songLowercase + ".json");
		}
	}
