...
class ResultsScreen extends MusicBeatSubstate
{
	...
	override function update(elapsed:Float)
	{
        ...
        if (PlayState.instance.health <= 0.01) // this may cause a bug, but I'd rather have the 1% of people who finish with this much health suffer than have that music bug if the health drain bug triggers on true trolling.
          music.stop();
        ...
        if (FlxG.keys.justPressed.ENTER)
        {
            ...
            if (PlayState.isStoryMode)
            {
              if (PlayState.SONG.song == "F.L.O.S.S"){
                //FlxTransitionableState.skipNextTransIn = true;
                //FlxTransitionableState.skipNextTransOut = true;
    						FlxG.switchState(new EndingState(PlayState.instance.accuracy >= 70));
