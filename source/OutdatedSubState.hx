...
class OutdatedSubState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var txt:FlxText = new FlxText(0, 0, FlxG.width,
			"Tux Trouble is outdated!\nYou are on "
			+ MainMenuState.tuxTroubleVer
			+ "\nwhile the most recent version is " + needVer + "."
			+ "\n\nWhat's new:\n\n"
			+ currChanges
			+ "\n& more changes and bugfixes in the full changelog"
			+ "\n\nPress Space to go to the GameBanana page\nor ESCAPE to ignore this",
			32);
	...
	override function update(elapsed:Float)
	{
		if (controls.ACCEPT)
		{
			fancyOpenURL("https://gamebanana.com/mods/286771");
		}
		if (controls.BACK)
		{
			// force game to not let you progress
			// if this is an outdated html5 reup
